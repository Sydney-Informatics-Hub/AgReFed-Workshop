...
1. Manually download data without the YAML configuration file
2. Access and view Google Earth Engine data
3. Extract samples from downloaded rasters
:::
...
Once the data has been downloaded as images, you may manually extract samples like how you would with any other GeoTIFF - we will discuss that in **Session 3**.
...
Your instructor will demonstrate more examples during the session.
...
This is the end of **Session 2**. In this session we looked at using manual methods to download data from various API sources and the GEE Catalog. We also briefly looked at how to plot the data using the `terra` package as well as the `plot()` function for objects created using the `download_*()` functions.
