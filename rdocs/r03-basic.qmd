...
The `harvest()` function used in the last workshop was a wrapper for several `download_*()` functions already available in `dataharvester` (see [source](https://github.com/Sydney-Informatics-Hub/dataharvester/blob/main/inst/python/harvest.py)). In this workshop, we willl show you how to call these functions directly for more control over the data download process.
...
We will also show you how to extract samples from downloaded rasters using the `extract_values()` function.
