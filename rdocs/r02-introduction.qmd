...
initialise_harvester(envname = "r-reticulate", earthengine = TRUE)
```
When you evaluate `initialise_harvester()`, a few actions are performed:

1. The function verifies that the environment `envname` is available and contains all the required dependencies. If it does not, it will install them for you.
2. The function will also try to authenticate to Google Earth Engine if `earthengine = TRUE`. This might involve opening a browser window and copying a code into the console.
...
target_sources:
  DEA: [landsat_barest_earth]
  # copy and add the new lines below ----------
  DEM: [DEM]
